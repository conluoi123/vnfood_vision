...
export default function ImagePreview({
  ...
  return (
    <div className="relative bg-zinc-950/40 backdrop-blur-2xl border border-white/5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_20px_40px_-15px_rgba(0,0,0,0.5)] rounded-3xl overflow-hidden flex flex-col flex-1 min-h-[380px]">
      {/* Floated custom badge over photo exactly like the image! */}
      {selectedDish && !isScanning && (
        <>
          <div className="absolute top-4 left-4 z-20 bg-black/40 backdrop-blur-xl border border-white/10 px-4 py-2 rounded-full flex items-center gap-2 shadow-lg">
            <div className="w-2 h-2 rounded-full bg-[#ff4f1d] animate-pulse" />
            <span className="text-[14px] text-zinc-100 font-medium tracking-wide">
              {selectedDish.foodName} {activeDishKey === "pho" && !customImage ? "(Beef)" : ""}
            </span>
          </div>
          <button 
            onClick={() => setShowGradCam(!showGradCam)}
            className="absolute top-4 right-4 z-20 bg-black/40 hover:bg-black/60 backdrop-blur-xl border border-white/10 px-4 py-2 rounded-full flex items-center gap-2 shadow-lg transition-all active:scale-95"
          >
            <span className="text-[13px] text-[#ff4f1d] font-medium tracking-wide">
      ...
      <div className="relative flex-1 bg-zinc-950 overflow-hidden group">
        <img
          ...
          className={`w-full h-full object-cover transition-transform duration-1000 ${
            isScanning ? "blur-md scale-105 brightness-50" : "scale-100 brightness-90 group-hover:scale-105"
          }`}
        />
