...
Do giới hạn dung lượng, toàn bộ dữ liệu ảnh và checkpoints tạm thời được lưu trữ trên **Shared Google Drive**:
...
- `checkpoints/`: Lưu trữ trọng số mô hình CV trung gian (.pth).

### 3. Lưu trữ & Triển khai Mô hình (Hugging Face Hub)
Các phiên bản mô hình hoàn chỉnh (đặc biệt là mô hình LLM Finetune có dung lượng lớn > 5GB) được lưu trữ tập trung tại Hugging Face Hub để dễ dàng gọi và suy luận (Inference):
- **Model Repo:** [htvien/llm_model_vnfood](https://huggingface.co/htvien/llm_model_vnfood)
- Định dạng trọng số: `.safetensors` (An toàn và tối ưu cho tốc độ load).

### 4. Nền tảng Huấn luyện
...
│   ├── 00_crawler.ipynb
│   ├── 0_setup_template.ipynb
│   ├── 01_eda.ipynb
│   ├── 02_labeling_setup.ipynb
│   ├── 03_pipeline.ipynb
│   ├── 04_training.ipynb
│   └── 05_llm_finetune.ipynb
